...
def add_title(img, text='add_text'):
    ...
    img = cv2.copyMakeBorder(
        img, 50, 0, 0, 0, cv2.BORDER_CONSTANT, value=color)
    if len(img.shape) == 2:
...
def HistEq():
    ...
    try:
        ...
        if request.form['mode'] == 'gray':
            ...
            cv2.imwrite(save_eq_dir, resize(np.concatenate([add_title(add_border(
                img), text='Origin'), add_title(add_border(dst), text='HistEq')], 1)))
        else:
            img = cv2.imread(save_dir, cv2.IMREAD_COLOR)
            dsts = he.MyMethod(img, 'color')
            save_eq_dir = os.path.join(file_dir, new_filename + '_eq.jpg')
            texts = ['Origin', 'RGB_HistEq', 'HSI_HistEq', 'MyMethod']
            cv2.imwrite(save_eq_dir,  resize(np.concatenate(
                [add_title(add_border(dsts[i]), text=texts[i]) for i in range(4)], 1)))
    except:
        pass
